<template>
  ...
  <v-data-table :headers="headers" :items="instances" :loading="loading" item-value="id"><template #item.actions="{ item }"><v-btn size="small" variant="text" @click="lifecycle(item.id, 'start')">{{ t('features.gateway.start') }}</v-btn><v-btn size="small" variant="text" @click="lifecycle(item.id, 'stop')">{{ t('features.gateway.stop') }}</v-btn><v-btn size="small" variant="text" @click="lifecycle(item.id, 'restart')">{{ t('features.gateway.restart') }}</v-btn><v-btn size="small" variant="text" @click="openAuth(item.id)">{{ t('features.gateway.authentication') }}</v-btn><v-btn v-if="item.source === 'managed'" size="small" color="error" variant="text" @click="remove(item.id)">{{ t('features.gateway.delete') }}</v-btn></template></v-data-table>

  <v-dialog v-model="createDialog" max-width="620"><v-card><v-card-title>{{ t('features.gateway.addConnection') }}</v-card-title><v-card-text><v-select v-model="draft.type" :items="adapterTypes" item-title="name" item-value="type" :label="t('features.gateway.adapterType')" @update:model-value="resetFields" /><v-text-field v-model="draft.id" :label="t('features.gateway.adapterId')" required /><v-text-field v-for="field in selectedType?.fields || []" :key="field.name" v-model="draft.config[field.name]" :label="fieldLabel(field)" :type="field.secret ? 'password' : field.type === 'url' ? 'url' : 'text'" :required="field.required" :hint="field.secret ? t('features.gateway.secretStoredHint') : undefined" persistent-hint /></v-card-text><v-card-actions><v-spacer /><v-btn @click="createDialog = false">{{ t('features.gateway.cancel') }}</v-btn><v-btn color="primary" :loading="saving" @click="create">{{ t('features.gateway.save') }}</v-btn></v-card-actions></v-card></v-dialog>

  <v-dialog v-model="authDialog" max-width="460"><v-card><v-card-title>{{ t('features.gateway.authentication') }}</v-card-title><v-card-text><div class="text-center"><QrCodeViewer v-if="auth.challenge?.qr_uri" :value="auth.challenge.qr_uri" :alt="t('features.gateway.authenticationQrAlt')" :size="220" /><v-progress-circular v-else-if="auth.status === 'waiting_user'" indeterminate color="primary" /><p class="mt-4">{{ authStatusLabel(auth.status) }}</p><p v-if="auth.challenge?.instructions">{{ auth.challenge.instructions }}</p><code v-if="auth.challenge?.verification_code">{{ auth.challenge.verification_code }}</code><p v-if="auth.reason" class="text-error">{{ auth.reason }}</p></div></v-card-text><v-card-actions><v-btn @click="cancelAuth">{{ t('features.gateway.cancelAuthentication') }}</v-btn><v-spacer /><v-btn color="primary" @click="startAuth">{{ t('features.gateway.startAuthentication') }}</v-btn><v-btn @click="authDialog = false">{{ t('features.gateway.close') }}</v-btn></v-card-actions></v-card></v-dialog>
</template>
...
function authStatusLabel(status: string): string { const keys: Record<string, string> = { not_required: 'authStatusNotRequired', waiting_user: 'authStatusWaitingUser', authenticated: 'authStatusAuthenticated', failed: 'authStatusFailed' }; return keys[status] ? t(`features.gateway.${keys[status]}`) : status; }
