<template>
  ...
  <v-dialog v-model="createDialog" max-width="620"><v-card><v-card-title>Add Gateway connection</v-card-title><v-card-text><v-select v-model="draft.type" :items="adapterTypes" item-title="name" item-value="type" label="Adapter type" @update:model-value="resetFields" /><v-text-field v-model="draft.id" label="Instance ID" required /><v-text-field v-for="field in selectedType?.fields || []" :key="field.name" v-model="draft.config[field.name]" :label="field.label" :type="field.secret ? 'password' : field.type === 'url' ? 'url' : 'text'" :required="field.required" :hint="field.secret ? 'Stored in the managed secret backend' : undefined" persistent-hint /></v-card-text><v-card-actions><v-spacer /><v-btn @click="createDialog = false">Cancel</v-btn><v-btn color="primary" :loading="saving" @click="create">Save & Start</v-btn></v-card-actions></v-card></v-dialog>
