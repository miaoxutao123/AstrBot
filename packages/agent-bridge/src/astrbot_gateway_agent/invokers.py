...
class CommandInvoker:
    def __init__(
        ...
        self.command, self.timeout, self.max_stdout, self.allowlist = (
            command,
            timeout,
            max_stdout,
            allowlist,
    ...
    async def invoke(self, value: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        try:
            stdout, _stderr = await asyncio.wait_for(
                process.communicate(raw), self.timeout
            )
        ...
        if not isinstance(result, Mapping):
            raise ValueError("agent command returned non-object JSON")
        return result
...
class HttpInvoker:
    ...
    async def invoke(self, value: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if not isinstance(result, Mapping):
            raise ValueError("agent HTTP endpoint returned non-object JSON")
        return result
