...
async def gateway_discover() -> dict[str, Any]:
    ...
    async with _client() as client:
        return dict((await client.discover()).raw)
...
async def gateway_list_adapters() -> list[dict[str, Any]]:
    ...
    async with _client() as client:
        return [dict(item) for item in await client.list_adapters()]
...
async def gateway_find_endpoints(**filters: str) -> list[dict[str, Any]]:
    ...
    async with _client() as client:
        return [dict(item) for item in await client.find_endpoints(**filters)]
...
async def gateway_get_capabilities(endpoint: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    async with _client() as client:
        return [dict(item) for item in await client.get_capabilities(endpoint)]
...
async def im_send_message(endpoint: dict[str, Any], text: str) -> dict[str, Any]:
    ...
    async with _client() as client:
        return dict(await client.send_text(endpoint, text))
...
async def im_reply_to(event: dict[str, Any], text: str) -> dict[str, Any]:
    ...
    async with _client() as client:
        return dict(await client.reply(GatewayEvent.from_wire(event), text))


@mcp.tool()
async def gateway_execute(
    endpoint: dict[str, Any], command_type: str, payload: dict[str, Any]
) -> dict[str, Any]:
    """Generic future-capability escape hatch subject to Gateway authorization."""
    async with _client() as client:
        return dict(await client.execute(endpoint, command_type, payload))
