...
3. Follow the registration link from the manifest, then the authenticated bootstrap link returned by registration. Do not guess paths.
4. Install `astrbot-gateway-agent` and, for proactive actions, `astrbot-gateway-mcp`.
5. Read `agent_integration`; create an Agent-owned command or HTTP wrapper that implements `astrbot.agent.invoke.v1` and returns structured `astrbot.agent.result.v1`.
