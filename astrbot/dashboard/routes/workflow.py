...
class WorkflowRoute:
    def __init__(self, core_lifecycle: AstrBotCoreLifecycle, db_helper: BaseDatabase):
        ...
        self.routes = {
            ...
            "/workflow/deploy/<workflow_id>": ("POST", self.deploy_workflow_as_provider),
            "/workflow/undeploy/<workflow_id>": ("POST", self.undeploy_workflow_provider),
        }
    ...
    async def list_workflows(self):
        """List all workflows."""
        try:
            workflows = self.db_helper.get_all_workflows()
    ...
    async def get_workflow(self, workflow_id: str):
        ...
        try:
            workflow = self.db_helper.get_workflow(workflow_id)
            if not workflow:
    ...
    async def save_workflow(self):
        ...
        try:
            ...
            self.db_helper.save_workflow(workflow)
    ...
    async def delete_workflow(self, workflow_id: str):
        """Delete a workflow."""
        try:
            self.db_helper.delete_workflow(workflow_id)
                ...
                .ok(
                    {
                        ...
                        "logs": result.execution_logs if hasattr(result, "execution_logs") else [],
                    }
                )
                .__dict__
            )
    ...
    async def undeploy_workflow_provider(self, workflow_id: str):
        ...
        try:
            ...
            return (
                Response().ok({"message": "工作流 Provider 已移除"}).__dict__
            )
        except Exception as e:
